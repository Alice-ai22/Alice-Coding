...
```bash
vibe start /path/to/project web-app
```
...
vibe task-template default /path/to/project/task.md
vibe task-template web-app /path/to/project/task.md
```

检查任务书质量：

```bash
vibe check-task /path/to/project/task.md
```

`vibe check-task` 会输出 100 分制质量评分。除了目标、要求、验收标准、验证方式和输出要求，它还会提示工作目录规则、执行边界、项目上下文和具体验证命令是否清楚。
...
执行计划中会包含任务书质量检查结果、风险评估和完成报告格式。任务书缺少关键段落时，`vibe exec` 会继续执行，但会提醒“needs work”。

生成运行报告：

```bash
vibe report --last-run --cwd /path/to/project
```
